...
function checkAndGoToS9() {
    ...
    if (!s8Input || s8Input.value.trim() === "") {
        alert("الرجاء إدخال رقم الموبايل للتاجر");
        return; // منع الانتقال
    }
    updateS9Inputs(); 
    showScreen('s9');
...
document.querySelectorAll('.bank-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (val === 0 || phoneVal === "") {
            alert("لا يوجد حساب");
            return; // إيقاف الدالة ومنع الانتقال
        }
...
if (nextBtnS9) {
    nextBtnS9.addEventListener('click', () => {
        ...
        if (amount === "" || isNaN(numericAmount) || numericAmount === 0) {
            alert("الرجاء إدخال مبلغ صحيح لإتمام العملية");
            return; // يمنع فتح المودال والانتقال للشاشة 10
        }
